...
using namespace WinRTXamlTestApplication::Common;
...
DependencyProperty^ RichTextColumns::ColumnTemplateProperty::get()
{
	...
	{
		TypeName thisType = { "Microsoft.UI.Xaml.Controls.RichTextColumns", TypeKind::Custom };
		TypeName dataTemplateType = { "Windows.UI.Xaml.DataTemplate", TypeKind::Metadata };
...
DependencyProperty^ RichTextColumns::RichTextContentProperty::get()
{
	...
	{
		TypeName thisType = { "Microsoft.UI.Xaml.Controls.RichTextColumns", TypeKind::Custom };
		TypeName richTextBlockType = { "Windows.UI.Xaml.Controls.RichTextBlock", TypeKind::Metadata };
...
Size RichTextColumns::MeasureOverride(Size availableSize)
{
	...
	{
		// TEMPORARY: Workaround for S_FALSE return from Append resulting in exceptions in C++ but not C# or VB.  Discussions are ongoing.
...
Size RichTextColumns::ArrangeOverride(Size finalSize)
{
	...
	while (childrenIterator->HasCurrent)
	{
		...
		if (!childrenIterator->MoveNext()) break;
	}
	Size resultingSize(maxWidth, maxHeight);
	return resultingSize;
}
