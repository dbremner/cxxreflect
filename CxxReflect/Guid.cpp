...
namespace CxxReflect {
    ...
    Guid::Guid(String const& guid)
    {
        ...
        if (!(iss >> *this >> std::ws) || !iss.eof())
            throw RuntimeError(L"Failed to parse GUID from string.");
    }

    Guid::Guid(Element0 m0, Element1 m1, Element2 m2)
    {
        Detail::RangeCheckedCopy(Detail::BeginBytes(m0), Detail::EndBytes(m0), _data.begin(), _data.end());
        Detail::RangeCheckedCopy(Detail::BeginBytes(m1.data() + m1.size()), Detail::EndBytes(m1.data()),
                                 _data.begin() + 4, _data.end());

        Detail::RangeCheckedCopy(m2.begin(), m2.end(), _data.begin() + 8, _data.end());
    }

    Guid::Guid(U4 m0, U2 m1a, U2 m1b, U1 m2a, U1 m2b, U1 m2c, U1 m2d, U1 m2e, U1 m2f, U1 m2g, U1 m2h)
    {
        Detail::RangeCheckedCopy(Detail::BeginBytes(m0),  Detail::EndBytes(m0),  _data.begin(),     _data.end());
        Detail::RangeCheckedCopy(Detail::BeginBytes(m1a), Detail::EndBytes(m1a), _data.begin() + 4, _data.end());
        Detail::RangeCheckedCopy(Detail::BeginBytes(m1b), Detail::EndBytes(m1b), _data.begin() + 6, _data.end());
    ...
    std::wostream& operator<<(std::wostream& os, Guid const& x)
    {
        ...
        // TODO if (std::swprintf(buffer.data(), buffer.size(),
