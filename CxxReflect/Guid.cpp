...
namespace CxxReflect {
    ...
    {
        std::copy(Detail::BeginBytes(m0), Detail::EndBytes(m0), _data.data());
        std::copy(reinterpret_cast<std::uint8_t const*>(m1.data()),
                  reinterpret_cast<std::uint8_t const*>(m1.data() + m1.size()),
                  _data.data() + 4);
    ...
    {
        std::copy(Detail::BeginBytes(m0), Detail::EndBytes(m0), _data.data());
        std::copy(Detail::BeginBytes(m1a), Detail::EndBytes(m1a), _data.data() + 4);
        std::copy(Detail::BeginBytes(m1b), Detail::EndBytes(m1b), _data.data() + 6);
