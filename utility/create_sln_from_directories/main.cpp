...
//
// The Solution configuration and platform management UI in Visual Studio is awful and is extremely
// difficult to use with the CxxReflect Solution because we have a large number of configurations
// (due to the incompatibility of /ZW and non-/ZW translation units) and because different projects
// only support a subset of the complete set of configurations.  Unloading or loading projects and
// adding or removing projects from the Solution can cause the IDE to mix up which projects get
// built under each configuration.
//
