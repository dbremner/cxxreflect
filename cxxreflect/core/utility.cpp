...
#include "cxxreflect/core/iterator.hpp"
...
//  * Namespace changed to cxxreflect::core::<unnamed>
//
//
namespace cxxreflect { namespace core { namespace {
    ...
    inline void sha1::process_byte(unsigned char byte)
    {
        ...
        if (bit_count_low == 0) {
            ++bit_count_high;
            if (bit_count_high == 0) {
                throw runtime_error(L"sha1 too many bytes");
            }
        }
    }
...
namespace cxxreflect { namespace core {
    ...
    auto compute_sha1_hash(const_byte_iterator const first, const_byte_iterator const last) -> sha1_hash
    {
        sha1 instance;
        instance.process_block(first, last);

        sha1_hash hash;
        instance.get_digest(reinterpret_cast<sha1::digest_type>(*hash.data()));

        sha1::digest_type hash_digest(reinterpret_cast<sha1::digest_type>(*hash.data()));
        std::transform(begin(hash_digest), end(hash_digest), begin(hash_digest), [&](std::uint32_t x) -> std::uint32_t
        {
            std::reverse(begin_bytes(x), end_bytes(x));
            return x;
        });
